...
class cat_value:
    ...
    def _return_all_cat_data(self,data:pd.DataFrame)->list:
        try:
            cat_col=[col for col in data.columns if str(data[col].dtypes).lower().startswith('o') or str(data[col].dtypes).lower().startswith('bool') or str(data[col].dtypes).lower().startswith('cat')]
            return cat_col
        except:
    ...
    def _label_encodeing(self,data1:pd.DataFrame)->pd.DataFrame:
        try:
            col=self._return_all_cat_data(data1)
            data=data1[col]
            label_encoede_col=[col for col in data.columns if data[col].nunique()>4  and data[col].nunique()/len(data)<=0.15]
    ...
    def _count_encoding(self,data1):
        try:
            col=self._return_all_cat_data(data1)
            data=data1[col]
            count_encoding_col=[col for col in data.columns if data[col].nunique()/len(data)>0.15 and data[col].nunique()/len(data)<=0.5]
